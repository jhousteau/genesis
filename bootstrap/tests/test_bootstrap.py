...
class TestBootstrapScript:
    ...
    def test_python_api_project_creation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["bash", str(script_path), project_name, "--type", "python-api", 
                 "--path", tmpdir, "--skip-git"],
                capture_output=True,
                text=True
            )
    ...
    def test_typescript_service_project_creation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["bash", str(script_path), project_name, "--type", "typescript-service",
                 "--path", tmpdir, "--skip-git"],
                capture_output=True,
                text=True
            )
    ...
    def test_cli_tool_project_creation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["bash", str(script_path), project_name, "--type", "cli-tool",
                 "--path", tmpdir, "--skip-git"],
                capture_output=True,
                text=True
            )
    ...
    def test_makefile_content_valid(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["bash", str(script_path), project_name, "--path", tmpdir, "--skip-git"],
                capture_output=True,
                text=True
            )
