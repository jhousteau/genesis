...
class TestHealthCheck:
    ...
    def test_run_all_checks(self):
        ...
        assert any(
            r.name == "check1" and r.status == HealthStatus.HEALTHY for r in results
        )
        assert any(
            r.name == "check2" and r.status == HealthStatus.DEGRADED for r in results
        )
    ...
    def test_overall_status_calculation(self):
        ...
        # All healthy = healthy
        health.add_check(
            "healthy1", lambda: CheckResult("healthy1", HealthStatus.HEALTHY)
        )
        health.add_check(
            "healthy2", lambda: CheckResult("healthy2", HealthStatus.HEALTHY)
        )
        assert health.get_overall_status() == HealthStatus.HEALTHY

        # Any degraded = degraded
        health.add_check(
            "degraded", lambda: CheckResult("degraded", HealthStatus.DEGRADED)
        )
        assert health.get_overall_status() == HealthStatus.DEGRADED

        # Any unhealthy = unhealthy (takes precedence)
        health.add_check(
            "unhealthy", lambda: CheckResult("unhealthy", HealthStatus.UNHEALTHY)
        )
    ...
    def test_get_summary(self):
        ...
        health.add_check(
            "healthy", lambda: CheckResult("healthy", HealthStatus.HEALTHY)
        )
        health.add_check(
            "degraded", lambda: CheckResult("degraded", HealthStatus.DEGRADED)
        )
