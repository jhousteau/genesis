...
from genesis.core.version import get_project_version, sync_version_to_files, bump_version
...
@click.argument("bump_type", type=click.Choice(["major", "minor", "patch", "alpha", "beta", "rc"]))
@click.option("--project", "-p", help="Project path (default: current directory)")
@click.option("--sync", is_flag=True, help="Auto-sync to other files (default: enabled)")
...
def bump_version_cmd(bump_type: str, project: Optional[str], sync: bool, dry_run: bool):
    ...
    try:
        ...
        click.echo("❌ tomli-w required for version bumping. Install with: pip install tomli-w", err=True)
        exit(1)
