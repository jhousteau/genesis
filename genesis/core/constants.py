...
def get_service_name() -> str:
    ...
    service_name = os.environ.get("GENESIS_SERVICE")
    if service_name:
        return service_name
    ...
    raise ValueError(
        "Cannot determine service name. Set GENESIS_SERVICE environment variable "
...
def get_environment() -> str:
    ...
    env_vars = ["GENESIS_ENV", "ENVIRONMENT", "ENV", "NODE_ENV"]
    ...
    raise ValueError(
        "Cannot determine environment. Set one of: "
        + ", ".join(env_vars)
    )
...
class AILimits:
    ...
    def get_max_worktree_files() -> int:
        ...
        value_str = os.environ.get("GENESIS_MAX_WORKTREE_FILES")
        if not value_str:
            raise ValueError("GENESIS_MAX_WORKTREE_FILES environment variable is required")
        try:
            value = int(value_str)
            if value <= 0 or value > 100:
                raise ValueError("GENESIS_MAX_WORKTREE_FILES must be between 1-100")
            return value
        except ValueError as e:
            raise ValueError(f"Invalid GENESIS_MAX_WORKTREE_FILES '{value_str}': {e}")
    
    @staticmethod
    def get_max_project_files() -> int:
        """Get maximum files per project for AI safety."""
        value_str = os.environ.get("GENESIS_MAX_PROJECT_FILES")
        if not value_str:
            raise ValueError("GENESIS_MAX_PROJECT_FILES environment variable is required")
        try:
            value = int(value_str)
            if value <= 0 or value > 1000:
                raise ValueError("GENESIS_MAX_PROJECT_FILES must be between 1-1000") 
            return value
        except ValueError as e:
            raise ValueError(f"Invalid GENESIS_MAX_PROJECT_FILES '{value_str}': {e}")
    
    @staticmethod
    def get_max_component_files() -> int:
        """Get maximum files per component for AI safety."""
        value_str = os.environ.get("GENESIS_MAX_COMPONENT_FILES")
        if not value_str:
            raise ValueError("GENESIS_MAX_COMPONENT_FILES environment variable is required")
        try:
            value = int(value_str)
            if value <= 0 or value > 100:
                raise ValueError("GENESIS_MAX_COMPONENT_FILES must be between 1-100")
            return value
        except ValueError as e:
            raise ValueError(f"Invalid GENESIS_MAX_COMPONENT_FILES '{value_str}': {e}")
...
class RetryDefaults:
    ...
    def get_max_attempts() -> int:
        ...
        return int(get_required_env("GENESIS_RETRY_MAX_ATTEMPTS"))
    
    @staticmethod
    def get_initial_delay() -> float:
        """Get retry initial delay."""
        return float(get_required_env("GENESIS_RETRY_INITIAL_DELAY"))
    
    @staticmethod
    def get_max_delay() -> float:
        """Get retry max delay."""
        return float(get_required_env("GENESIS_RETRY_MAX_DELAY"))
    
    @staticmethod
    def get_exponential_base() -> float:
        """Get retry exponential base."""
        return float(get_required_env("GENESIS_RETRY_EXPONENTIAL_BASE"))
...
class CircuitBreakerDefaults:
    ...
    def get_failure_threshold() -> int:
        ...
        return int(get_required_env("GENESIS_CB_FAILURE_THRESHOLD"))
    
    @staticmethod
    def get_timeout() -> float:
        """Get circuit breaker timeout."""
        return float(get_required_env("GENESIS_CB_TIMEOUT"))
    
    @staticmethod
    def get_half_open_max_calls() -> int:
        """Get circuit breaker half-open max calls."""
        return int(get_required_env("GENESIS_CB_HALF_OPEN_MAX_CALLS"))
    
    @staticmethod
    def get_success_threshold() -> int:
        """Get circuit breaker success threshold."""
        return int(get_required_env("GENESIS_CB_SUCCESS_THRESHOLD"))
    
    @staticmethod
    def get_sliding_window_size() -> int:
        """Get circuit breaker sliding window size."""
        return int(get_required_env("GENESIS_CB_SLIDING_WINDOW_SIZE"))
...
class LoggerConfig:
    ...
    def get_level() -> str:
        ...
        env = os.environ.get("GENESIS_LOG_LEVEL")
        if env:
            return env
        ...
        raise ValueError(
            "Log level not configured. Set GENESIS_LOG_LEVEL or configure environment."
        )
    
    @staticmethod
    def should_format_json() -> bool:
        """Whether to format logs as JSON."""
        value = os.environ.get("GENESIS_LOG_JSON", "").lower()
    ...
    def should_include_timestamp() -> bool:
        ...
        value = os.environ.get("GENESIS_LOG_TIMESTAMP", "").lower()
        if value in ["true", "1", "yes"]:
            return True
        elif value in ["false", "0", "no"]:
            return False
    ...
    def should_include_caller() -> bool:
        ...
        value = os.environ.get("GENESIS_LOG_CALLER", "").lower()
        if value in ["true", "1", "yes"]:
            return True
        elif value in ["false", "0", "no"]:
            return False
...
def get_genesis_components() -> dict[str, str]:
    ...
    components_json = os.environ.get("GENESIS_COMPONENTS")
    if components_json:
        import json
        try:
            return json.loads(components_json)
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid GENESIS_COMPONENTS JSON: {e}")
    
    # Try individual environment variables
    component_vars = {
        f"GENESIS_COMPONENT_{name.upper().replace('-', '_')}": name 
        for name in [
            "bootstrap", "smart-commit", "worktree-tools", 
            "shared-python", "genesis-cli", "testing"
        ]
    }
    ...
    raise ValueError(
        "No Genesis components configured. Set GENESIS_COMPONENTS (JSON) or "
        "individual GENESIS_COMPONENT_* environment variables"
    )
...
def get_component_scripts() -> dict[str, str]:
    ...
    """
    # Try environment variable first (JSON format)  
    scripts_json = os.environ.get("GENESIS_COMPONENT_SCRIPTS")
    if scripts_json:
        import json
        try:
            return json.loads(scripts_json)
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid GENESIS_COMPONENT_SCRIPTS JSON: {e}")
    
    # Try individual environment variables
    script_vars = {
        "GENESIS_WORKTREE_SCRIPT": "worktree-tools",
        "GENESIS_SMART_COMMIT_SCRIPT": "smart-commit",
    }
    ...
    if not configured_scripts:
        raise ValueError(
            "No component scripts configured. Set GENESIS_COMPONENT_SCRIPTS (JSON) or "
            "individual GENESIS_*_SCRIPT environment variables"
        )
