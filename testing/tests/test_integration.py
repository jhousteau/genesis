...
class TestComponentIntegration:
    ...
    def test_bootstrap_to_worktree_workflow(self, temp_dir):
        ...
        with patch_subprocess_run()[0] as mock_run:
            ...
            import sys
            sys.path.insert(0, str(Path(__file__).parent.parent.parent / "genesis-cli" / "src"))
            ...
            with patch('genesis.find_genesis_root', return_value=temp_dir):
                ...
                result = runner.invoke(cli, [
                    'worktree', 'fix-auth', 'src/auth/login.py'
                ])
                
                assert result.exit_code == 0
                assert "Sparse worktree 'fix-auth' created successfully" in result.output
    ...
    def test_component_script_discovery(self, temp_dir):
        ...
        worktree_script = temp_dir / "worktree-tools" / "src" / "create-sparse-worktree.sh"
...
class TestEndToEndWorkflows:
    ...
    def test_ai_safety_throughout_workflow(self, temp_dir):
        ...
        for component in ['bootstrap', 'genesis-cli', 'smart-commit', 
                         'worktree-tools', 'shared-python']:
            component_path = temp_dir / component
            component_result = checker.check_component(component_path)
            assert component_result['is_safe'], f"Component {component} unsafe after workflow"
    ...
    def test_cli_error_propagation(self, temp_dir):
        ...
        with patch('genesis.find_genesis_root', return_value=temp_dir):
            ...
            with patch('subprocess.run') as mock_run:
                mock_run.side_effect = subprocess.CalledProcessError(1, 'bootstrap', 'Script failed')
