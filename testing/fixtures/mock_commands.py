...
class MockCommandRunner:
    ...
    def configure_command(self, command: str, returncode: int = 0,
    ...
    def get_calls_matching(self, command_pattern: str) -> List[Dict[str, Any]]:
        ...
        return [call for call in self.call_history if command_pattern in call['command']]
...
def create_genesis_script_mocks() -> MockCommandRunner:
    ...
    # Bootstrap script
    runner.configure_command("bootstrap.sh", returncode=0,
                           stdout="🚀 Project Bootstrap\n✅ Bootstrap complete!")
    
    # Smart commit script
    runner.configure_command("smart-commit.sh", returncode=0,
                           stdout="🔍 Running quality checks\n✅ Commit successful!")
    
    # Worktree script
    runner.configure_command("create-sparse-worktree.sh", returncode=0,
...
class MockScriptEnvironment:
    ...
    def patch_all(self):
        ...
        return patch.multiple(
            'subprocess',
            run=self.command_runner.mock_run
        )
