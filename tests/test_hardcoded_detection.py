...
REDIS_URL = "redis://127.0.0.1:6379"
    ...
    def test_typescript_function_defaults_detected(self, variable_defaults_script: Path):
...
class Config:
    ...
    def test_typescript_object_properties_detected(self, variable_defaults_script: Path):
...
const API_PORT = ":8080";
    ...
    def test_acceptable_values_ignored(self, variable_defaults_script: Path):
        """Test that acceptable default values are not flagged."""
        test_files = [
            ("acceptable.py", '''
def good_function(enabled=True, count=0, items=None, debug=False):
    pass

def another_good(value=1, negative=-1):
    pass

# Version info is acceptable
__version__ = "1.0.0"
__author__ = "Author Name"
            '''),
            ("acceptable.ts", '''
function goodFunction(enabled = true, count = 0, items = null) {
    return enabled;
}

const arrowGood = (value = 1, negative = -1) => value;

// Package info is acceptable  
const version = "1.0.0";
...
MONGO_URL = "mongodb://admin:secret@mongo:27017/app"
    ...
    def test_environment_variable_names_detected(self, variable_defaults_script: Path):
        """Test detection of hardcoded environment variable names."""
        test_files = [
            ("env_names.py", '''
# Bad - hardcoded env var names as strings
...
service_var = "SERVICE_NAME"
        ...
        # Should detect issues
        assert returncode != 0
        assert "Hardcoded environment variable names" in stdout

    def test_clean_codebase_passes(self, hardcoded_script: Path, variable_defaults_script: Path):
...
if (!DATABASE_URL) {
    throw new Error("DATABASE_URL environment variable is required");
        ...
        returncode2, stdout2, stderr2 = self.run_script(variable_defaults_script, test_dir)
